...
export default function Menu({ item }) {
  return (
    <Link to={`/QuickBite/menu/${item._id}`} >
      <div className="w-72 border border-gray-300 rounded-lg overflow-hidden shadow-lg m-5 transition-transform transform hover:-translate-y-1 flex flex-col" key={item._id}>
        <img
          src={item.image}
          alt="food image"
        ...
        <div className="p-4 flex flex-col flex-grow">
          ...
          <p className="text-sm text-gray-700 mb-4 overflow-hidden" style={{ maxHeight: '3rem' }}>
            {item.description}
          </p>
